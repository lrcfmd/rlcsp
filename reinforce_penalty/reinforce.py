...
def test(filename):
    ...
    algo = Reinforce(params_db={'host':'localhost',
                                'user':'root',
                                'password':'MyRootPassword'},
                                actions=['T1', 'T2', 'T3', 'T4', 'T5', 'T6'], alpha=0.01, debug=True)
    # algo = Reinforce(params_db=db_file,
    #                  reinforce_table='reinforce_params_1',
    #                  theta_table='theta_table_1',
    #                  alpha=0.01,
    #                  reward_type=Reinforce.change_in_features,
    #                  features_set=[Reinforce.f_energy],
    #                  episode_length=1)

    simulator = ActionSimulator()
    simulator.add_dataset(str(pathlib.Path(__file__).parent.absolute()) + '/big_simulation_dataset.csv')

    old_state = simulator.random_state()
    old_state.old_energy = old_state.energy

    N = 20000

    zero_reward_five = 0
    zero_reward_ten = 0
    counter = {k : [0, 0] for k in algo.actions}

    for i in range(N):
        if len(old_state.struct) is max([5,10]):
            excluded_actions = ['T5']
        else:
            excluded_actions = []
        # print(excluded_actions)
        action = algo.select_action(state=old_state, excluded_actions=excluded_actions)
        # print('Stack length is %d, action is %s' % (len(old_state.struct), action))
        if i % 200 == 0:
            print('Step is ' + str(i))
            print('Features for action ' + action + ': ' + str(algo.features(old_state, action)))
            print('Theta is ' + str(algo.theta))
        if action == 'T5' and old_state.sl() == 10:
            new_state = State(energy=old_state.energy + 0.2, struct=old_state.struct)
            new_state.old_energy = old_state.energy
            outcome = 'A'
        else:
            new_state, outcome = simulator.simulate_action(state=old_state, action=action, min_actions_num=5)
        # print(old_state.energy - new_state.energy)
        if (old_state.energy - new_state.energy) != 0.0:
            algo.update(action=action, old_state=old_state, new_state=new_state)
            # algo.print_probabilities(State(-25.10))
            # algo.print_probabilities(State(-25.20))
            counter[action][int(old_state.sl()/5-1)] += 1
            if outcome != 'cR' and new_state.energy < algo.max_energy:
                old_state = new_state
            # print(algo.theta)
        else:
            if (old_state.energy - new_state.energy) == 0.0:
                print('reward is zero')
                if new_state.sl() == 5:
                    zero_reward_five += 1
                else:
                    zero_reward_ten += 1
    ...
    print('Zero rewards with sl=5: ' + str(zero_reward_five))
    print('Zero rewards with sl=10: ' + str(zero_reward_ten))
    print('The number of performed actions: ' + str(counter))

    print("THETA IS ")
    print(algo.theta)
    print('The mean energy is ')
    print(algo.f_mean[Reinforce.f_energy])
    algo.draw_actions_probabilities(-25.25, -25.0)

    algo.animated_actions_probabilities(-25.25, -25.0, interval=100, filename=filename)
    # algo.animated_actions_probabilities(-25.25, -25.0, interval=100, filename='10_high_' + filename, sl=[10], old_energy=-25.0)
    # algo.animated_actions_probabilities(-25.25, -25.0, interval=100, filename='10_' + filename, sl=[10])
    # algo.animated_actions_probabilities(-25.25, -25.0, interval=100, filename='510_' + filename, sl=[5,10])


def alpha_test(filename, checked_action='T3', alpha=0, simulator=None):

    print('Start alpha_test with alpha=' + str(alpha))

    algo = []

    for k in range(5):

        db_file = f'{k}reinforce10.db'
        if os.path.exists(db_file):
            os.remove(db_file)
        algo.append(Reinforce(params_db=db_file, alpha=alpha, debug=False))

    N = 10000

    if simulator is None:
        simulator = ActionSimulator()
        simulator.add_dataset('context_bandit/big_simulation_dataset.csv')
    ...
    for i in range(len(algo)):
        ...
        old_state = simulator.random_state()
        if old_state.energy > 0:
            old_state.energy = -25.13

        old_state.old_energy = old_state.energy

        print('Start algo ' + str(i))
        # print("Start algo with alpha=" + str(algo[i].alpha))
        batch = []
        for j in range(N):
            ...
            action = algo[i].select_action(state=old_state, excluded_actions=excluded_actions)

            new_state, outcome = simulator.simulate_action(state=old_state, action=action, min_actions_num=5)
            if outcome != 'cR' and new_state.energy < 0:
                batch.append([action, old_state, new_state])
                old_state = new_state
                if len(batch) >= 40:
                    # print('Update batch')
                    for run in batch:
                        # print(run)
                        algo[i].update(action=run[0], old_state=run[1], new_state=run[2])
                    batch = []
            if j == 10000:
                print('After 10000 steps: ')
                print("Theta: " + str(algo[i].theta))
                algo[i].print_probabilities(State(-25.0))

        print('Mean energy: ' + str(algo[i].f_mean[Reinforce.f_energy]))
        print("Theta: " + str(algo[i].theta))
        algo[i].print_probabilities(State(-25.0))

        mean_prob += algo[i].action_prob(action='T6', state=State(-25.0))

    mean_prob /= len(algo)
    print('Mean probability for T6 action is: ' + str(mean_prob))
    ...
    actions_num = len(algo[0].actions)
    min_energy = -25.25
    max_energy = -25.0
    interval = 100
    ...
    lines = [0] * len(algo)
    for i in range(len(algo)):
        lines[i], = plt.plot([min_energy, max_energy], [0, 100])

    len_thetas = N
    for a in algo:
        len_thetas = min(len_thetas, len(a.thetas))

    T = np.linspace(1, len_thetas, int(len_thetas / interval))
    # print(T)

    temp = ax.text(int((min_energy + max_energy) / 2), 100, '', ha='right', va='top', fontsize=24)

    # Get colors from coolwarm colormap
    colors = plt.get_cmap('hsv', len(algo))
    ...
    def animate(i):
        ...
        for a in range(len(algo)):
            y = np.ones(len(x))
            for j in range(len(x)):
                state = State(x[j], old_energy=x[j])
                y[j] = 100 * Reinforce.st_action_prob(checked_action,
                                                      state,
                                                      algo[a].thetas[int(T[i] - 1)],
                                                      algo[a].actions,
                                                      algo[a].f_mean,
                                                      algo[a].f_std,
                                                      algo[a].f_max,
                                                      ...
                                                      algo[a].f_array)
            lines[a].set_data(x, y)
            lines[a].set_label(algo[a].alpha)
            lines[a].set_color(colors(a))
    ...
    print(f'save animation to {filename}')
    # Save and show animation
    ani.save(filename, writer=PillowWriter(fps=2))
    # plt.show()
...
def mean_energy_test(filename, checked_action='T6'):

    alpha=0.15

    print('Start alpha_test with alpha=' + str(alpha))

    algo = [Reinforce(alpha=alpha) for k in range(10)]
    ...
    for i in range(len(algo)):
        ...
        for j in range(N):
            ...
            new_state, outcome = simulator.simulate_action(state=old_state, action=action, min_actions_num=5)
            # sim_action_timer += time.perf_counter_ns() - start
            if outcome != 'cR' and new_state.energy < 0:
                # start = time.perf_counter_ns()
                algo[i].update(action=action, old_state=old_state, new_state=new_state)
                # update_timer += time.perf_counter_ns() - start
                old_state = new_state
                energies[i].append(algo[i].mean_energy)

        print('Mean energy: ' + str(algo[i].mean_energy))
        print("Theta: " + str(algo[i].theta))
        # print('Simulate action time: ' + str(sim_action_timer))
        # print('Update time: ' + str(update_timer))
        algo[i].print_probabilities(State(-25.0))
        # return
    ...
    lines = [0] * len(algo)
    for i in range(len(algo)):
        lines[i], = plt.plot([0, N], [min_energy+0.05, max_energy-0.05])
    ...
    def animate(i):
        for a in range(len(algo)):
            x = range(0, len(energies[a]))
            lines[a].set_data(x, energies[a])
            lines[a].set_color(colors(a))
    ...
    # Create animation
    ani = FuncAnimation(fig=fig, func=animate, frames=[0], interval=50, repeat=True)
...
def reward_test(filename, checked_action='T6'):
    print('Start reward test')

    db_file = 'theta_params.db'
    if os.path.exists(db_file):
        os.remove(db_file)

    algo = [Reinforce(alpha=0.01, params_db=db_file, reward_type=Reinforce.change_in_features)]
            # Reinforce(alpha=0.01, reward_type=Reinforce.standard_change_in_features)]
    ...
    old_state = simulator.random_state()
    action = algo[0].select_action(state=old_state)
    ...
    for i in range(len(algo)):
        ...
        for j in range(N):
            ...
            action = algo[i].select_action(state=old_state)
            while action == 'T5' and old_state.sl() == 10:
                action = algo[i].select_action(state=old_state)
            ...
            if outcome != 'cR' and new_state.energy < 0:
                ...
                if len(batch) >= 40:
                    ...
                    for run in batch:
                        # print(run)
                        algo[i].update(action=run[0], old_state=run[1], new_state=run[2])
                        std.append(algo[i].f_std[Reinforce.f_energy])
                    batch = []
            if j == 10000:
                print('After 10000 steps: ')
                print("Theta: " + str(algo[i].theta))
    ...
    # Get colors from coolwarm colormap
    colors = plt.get_cmap('hsv', len(algo) + 3)
    ...
    for a in range(len(algo)):
        y = algo[a].rewards
        line, = plt.plot(y, linewidth=1)
        line.set_label('rewards')
        line.set_color(colors(a))
        plt.legend()
    ...
    def animate(i):
        ...
        for a in range(len(algo)):
            y = np.ones(len(x))
            for j in range(len(x)):
                y[j] = 100 * Reinforce.st_action_prob(checked_action, State(x[j]),
                                                      algo[a].thetas[int(T[i] - 1)],
                                                      algo[a].actions,
                                                      algo[a].f_mean[Reinforce.f_energy],
                                                      algo[a].f_std[Reinforce.f_energy])
            lines[a].set_data(x, y)
            lines[a].set_label(algo[a].reward_type)
            lines[a].set_color(colors(a))
...
def parallel_runs_test(filename, runs_num=40, alpha=0):

    db_filename = '40runs_params.db'

    if os.path.exists(db_filename):
        os.remove(db_filename)

    print('Start parallel_runs_test with alpha=' + str(alpha) + ' and runs num=' + str(runs_num))

    algo = [Reinforce(alpha=alpha,
                      reward_type=Reinforce.change_in_features,
                      params_db=db_filename,
                      reinforce_table='reinforce_' + str(k)) for k in range(runs_num)]

    N = 500

    simulator = ActionSimulator()
    simulator.add_dataset('big_simulation_dataset.csv')

    old_states = [0 for k in range(runs_num)]
    new_states = [0 for k in range(runs_num)]

    for step in range(N):

        for i in range(runs_num):

            old_states[i] = simulator.random_state()

            action = algo[i].select_action(state=old_states[i])
            new_states[i], outcome = simulator.simulate_action(state=old_states[i], action=action, min_actions_num=5)
            if outcome != 'cR' and new_states[i].energy < 0:
                algo[i].update(action=action, old_state=old_states[i], new_state=new_states[i])

        print("Theta: " + str(algo[0].theta))
        algo[0].print_probabilities(State(-25.0))

    for i in range(runs_num):
        print(algo[i].mean_energy)

    print("Draw plot")

    algo[0].draw_actions_probabilities(-25.25, -25.0)

    algo[0].animated_actions_probabilities(-25.25, -25.0, interval=50, filename=filename)


def mc_loop_test(filename, reg_params, red_T=0.025, fixed_episode=True, simulator=None):
    db_file = 'reinforce10.db'
    if os.path.exists(db_file):
        os.remove(db_file)

    algo = Reinforce(params_db={'host': 'localhost',
                                'database': 'beta_test28',
                                'user': 'root',
                                'password': 'MyRootPassword'},
                     theta_table='theta_penalty_08_' + str(beta).replace('.', '_'),
                     reinforce_table='reinforce_penalty_08_' + str(beta).replace('.', '_'),
                     actions=['T1', 'T2', 'T3', 'T4', 'T5', 'T6'],
                     alpha=0.01,
                     fixed_episodes=fixed_episode,
                     reward_type=Reinforce.change_in_features,
                     debug=True,
    ...
    for i in range(N):
        if len(old_state.struct) is max([5, 10]):
            ...
            excluded_actions = []
        # print(excluded_actions)
        # old_state.energy = random.choice([-25.05, -25.10, -25.0, -25.15, -25.12, -25.8, -25.12])
        action = algo.select_action(state=old_state, excluded_actions=excluded_actions)
        # actions = algo.actions.copy()
        ...
        # start mc-loop
        diff = new_state.energy - old_state.energy
        accept = False
        p = random.random()
        r += 1
        if diff < 0:
            ...
            # print(math.exp(-diff/(float(red_T))))
            if math.exp(-diff/(float(red_T))) >= p and new_state.energy < algo.max_energy:
                accept = True

        # if we accept the structure change old_structure and update reinforce
        if accept:
            algo.update(action=action, old_state=old_state, new_state=new_state)
            old_state = new_state
        else:
            if fixed_episode:
                ...
                # if we do not accept the structure: end the episode, update reinforce and end the short episode
                algo.end_episode()
                algo.update(action=action, old_state=old_state, new_state=new_state, end_episode=True)

        # always accept energy test
        if new_state.energy < 0:
            old_state = new_state
    ...
    print('The mean energy is ')
    print(algo.f_mean[Reinforce.f_energy])
    algo.draw_actions_probabilities(-25.22, -25.0, filename=f'actions_prob_{beta}.png')

    algo.animated_actions_probabilities(-25.22, -25.0, interval=100, filename=filename)

    plt.plot(energies, linewidth=1)
